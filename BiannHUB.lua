--[[
    BIANNHUB RECORDER v1.0 - FINAL FIX
    - Indikator iPhone bisa diklik
    - Tanpa error nil
]]
...
local function createRecordingIndicator()
    ...
    indicator.Name = "RecordIndicator"
    indicator.Size = UDim2.new(0, 44, 0, 44)
    indicator.Position = UDim2.new(0, 16, 0.5, -22)
    indicator.AnchorPoint = Vector2.new(0, 0.5)
    indicator.BackgroundColor3 = Color3.fromRGB(255, 40, 40)
    ...
    task.spawn(function()
        while indicator and indicator.Parent do
            ...
            if blinkState then
                ...
                indicator.BackgroundColor3 = Color3.fromRGB(200, 20, 20)
            end
            task.wait(0.4)
        end
    end)
...
local function getCompressedFrames(originalFrames)
    ...
    local newFrames = {}
    for i, frame in ipairs(originalFrames) do
    ...
    if #newFrames < 2 then return originalFrames end
    ...
    for i, frame in ipairs(newFrames) do
        ...
        if i < #newFrames then
            local nextFrame = newFrames[i+1]
            local delta = (nextFrame.time - frame.time) * 0.5
...
local function createGUI()
    ...
    titleLbl.Text = "📼 BiannHUB Recorder"
    ...
    task.spawn(function()
        while gui.Parent do
            ...
            elseif isWalking and currentWalk then
                ...
                recStatus.Text = "✅ Siap | F = Start/Stop Record"
                recStatus.TextColor3 = Color3.fromRGB(100,255,150)
            end
            task.wait(0.2)
        end
